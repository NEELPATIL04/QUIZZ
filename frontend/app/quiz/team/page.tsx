...
export default function TeamQuizPage() {
  ...
  useEffect(() => {
    ...
    // Poll for new questions and team score every 3 seconds
  ...
  const handleSubmitAnswer = async (questionId: string) => {
    if (!teamNumber || !answers[questionId]?.trim()) {
        ...
        <>
          ...
          {currentQuestionIndex < questions.length && (() => {
            ...
            if (q.questionType === 'broken_html_challenge') {
              ...
              const treeStructure = q.treeStructure ? JSON.parse(q.treeStructure) : {};
              const correctTree = q.correctTree ? JSON.parse(q.correctTree) : {};
